...
class NannyToolbar extends React.Component {
  ...
  delete = (editor) => {
      ...
      if(editor === 'manifest')
      {
        this.props.actions.manifest.deleteManifest(this.props.state.auth, this.props.state.nannyEditor.id);
        initNewEditor(this.props.actions.nannyEditor.SetNannyEditor, editor);
        this.props.actions.app.setTitle('');
      }
  }
  ...
  handleChange = (event, index, value, editor) => {
    console.log(index);
    if (index === -2) {
      return;
    } else {
      if (value === -1) {
        return this.resetEditor(editor);
      }
      else {
        if (editor === 'configuration') {
